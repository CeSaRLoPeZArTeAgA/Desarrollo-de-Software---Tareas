...
class SignatureServiceAdapter:
    ...
    def extraer_id_documento_firmado(self, contenedor_url: str) -> int:
        ...
        if not match:
            raise HTTPException(
                ...
                detail="No se pudo identificar el documento firmado desde la URL del contenedor.",
            )
    ...
    def descargar_bytes(self, url: str) -> bytes:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"No se pudo descargar el archivo desde signature-service: {url}",
            ) from exc
...
class EntidadSimuladaFacade:
    ...
    def listar_documentos(self, dependencia: str) -> list[DocumentoEntidadResponse]:
        rows = self.repository.listar_por_dependencia(dependencia.strip())
        return [DocumentoEntidadResponse(**row) for row in rows]

    def verificar_firma(self, tramite_id: int) -> VerificacionFirmaResponse:
        ...
        detalle = self.signature_adapter.verificar_documento_guardado(signed_document_id)
    ...
    def generar_zip_documento(self, tramite_id: int) -> tuple[str, bytes]:
        documento = self._obtener_documento_o_404(tramite_id)
        ...
        contenedor_bytes = self.signature_adapter.descargar_bytes(documento["contenedor"])
    ...
    def borrar_documento(self, tramite_id: int) -> None:
        self._obtener_documento_o_404(tramite_id)
...
async def listar_documentos_por_dependencia(
    dependencia: str = Query(..., min_length=1),
) -> list[DocumentoEntidadResponse]:
    return get_facade().listar_documentos(dependencia)
...
async def verificar_firma_documento(tramite_id: int) -> VerificacionFirmaResponse:
    return get_facade().verificar_firma(tramite_id)


@router.get("/documentos/{tramite_id}/download-paquete")
async def descargar_paquete_documento(tramite_id: int) -> Response:
    filename, zip_bytes = get_facade().generar_zip_documento(tramite_id)
...
async def borrar_documento_tramitado(tramite_id: int) -> Response:
    get_facade().borrar_documento(tramite_id)
